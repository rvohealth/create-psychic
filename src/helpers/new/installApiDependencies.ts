...
export default async function installApiDependencies(
  ...
  switch (options.packageManager) {
    case 'yarn':
      await sspawn(
        `cd ${apiRoot} && touch ${lockfileName} && corepack enable yarn && yarn set version stable && yarn install && yarn add @rvoh/dream @rvoh/psychic && yarn psy sync:ai-rules all`,
        {
          onStdout: message => {
    ...
    case 'pnpm':
      await sspawn(
        `cd ${apiRoot} && corepack enable pnpm && pnpm install && pnpm add @rvoh/dream @rvoh/psychic && pnpm run psy sync:ai-rules all`,
        {
          onStdout: message => {
            logger.logContinueProgress(colorize('[api]', { color: 'cyan' }) + ' ' + message, {
              logPrefixColor: 'cyan',
    ...
    case 'npm':
      await sspawn(
        `cd ${apiRoot} && touch ${lockfileName} && npm install && npm install @rvoh/dream @rvoh/psychic && npm run psy sync:ai-rules all`,
        {
          onStdout: message => {
            logger.logContinueProgress(colorize('[api]', { color: 'cyan' }) + ' ' + message, {
              logPrefixColor: 'cyan',
