...
A.I. rules for developing a Dream and Psychic application are provided in `api/.cursor/rules`, `api/CLAUDE.md`, and `api/.github/copilot-instructions.md` (`.cursor/rules`,`CLAUDE.md`, and `.github/copilot-instructions.md` if this Psychic app was created without a front-end client). These rules may be customized by adding to the bottom of the file (a rule exists in the file to instruct A.I. to add new rules outside of the official Psychic rules).

To fetch the current version of the A.I. rules, use `yarn psy sync:ai-rules <claude|copilot|cursor|all>`.
