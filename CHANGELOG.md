...
- Remove the dead `@types/koa-etag` dependency from the boilerplate and the `init` installer. The generated app (and psychic) imports `@koa/etag`, which ships its own types; `koa-etag` is never imported, so its `@types` package was unused. (The `koa-bodyparser` → `@koa/bodyparser` swap is intentionally deferred until `@rvoh/psychic` 3.6.0 — which makes that migration — is published, since the currently-published psychic still imports `koa-bodyparser`.)
