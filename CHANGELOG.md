...
- CI (`pr-checks.yml`) now installs Bun (`oven-sh/setup-bun`) in the spec job so the bun-runtime generation specs (`describe.skipIf(!bunAvailable())`) actually run instead of silently skipping. Deno is intentionally NOT installed in CI — its generated apps hit the SWC field-decorator boot bug (see `docs/deno-runtime-readiness.md`), so the deno specs stay skipped until that clears. CI-only; no source-tree or generated-output impact.
- Boilerplate `api/src/conf/initializers/websockets.ts` now selects the websockets transport adapter per environment (à la Rails `cable.yml`): `test` uses the in-process adapter (no Redis — hermetic unit specs, with real in-process delivery still available to feature specs), while `development`/`production` keep the Redis adapter. The Redis connection is now configured only outside of `test` (gated behind `!AppEnv.isTest`), so generated apps no longer open a Redis socket during `test`. Requires `@rvoh/psychic-websockets` ≥ 3.2.0 (the release that introduces the adapter seam); the boilerplate dependency floor is raised accordingly.
