...
- Fix: a Next.js client app (`--template=nextjs`) now builds cleanly with its generated placeholder model spec in place. `create-next-app` produces a single `tsconfig.json` that also drives `next build`'s own typecheck, and its `include` sweeps up every `.ts` file, including `spec/`. The freshly generated model spec stub relies on vitest's ambient `describe`/`it` globals rather than importing them explicitly, so `next build` failed with `error TS2582: Cannot find name 'describe'` the moment a client was added — on every package manager (npm, pnpm, yarn), for both the plain and `--dream-only` template variants. `TsConfigBuilder` now adds `"vitest/globals"` to `compilerOptions.types` when merging path aliases into an already-existing tsconfig.json (the Next.js case), so `next build`'s typecheck resolves the same globals vitest itself provides at test time. The api-only boilerplate's own tsconfig split (a build-only `tsconfig.build.json` whose `include` omits `spec`, plus a separate dev-time `tsconfig.json`) is untouched and unaffected.
...
- Fix the generated CI screenshot artifact name when sharded: `feature-spec-screenshots-${{ matrix.shard }}` embeds the shard label (e.g. `1/1`), and `/` is invalid in GitHub artifact names, so the upload errored exactly when a sharded run failed. The name now uses `${{ strategy.job-index }}`. The upload step also gains `if-no-files-found: ignore` (a failure before the browser launches produces no screenshots), and the redundant `mkdir -p /tmp/screenshots` step is removed — the hooks create the directory on demand.
- Bump boilerplate `@rvoh/dream-spec-helpers` to `^2.3.0` and switch both generated hooks files (`api/spec/unit/setup/hooks.ts`, `api/spec/features/setup/hooks.ts`) from `truncate` to its new name, `cleanTestDb`. 2.3.0 replaces the per-test TRUNCATE-every-table loop with automatic dirty-table detection: each call probes which tables actually contain rows and cleans only those (children-first batched DELETE; FK-cycle groups via a grouped TRUNCATE), taking a read-only test's cleaning cost from ~tens of milliseconds to well under a millisecond. `truncate` remains a deprecated alias, so existing generated apps get the speedup on upgrade with no code change. Note one behavior change for spec authors: queries without an ORDER BY no longer coincidentally return rows in insertion order — declare an order or assert order-agnostically.
...
- The interactive prompt is now a single `install psychic-skill?` question with `yes (recommended)` / `no` options. Answering yes installs the project skill for both `.agents`-compatible agents and Claude Code, using the unified single-copy shape above.
...
- Generated CI workflows now include `CORS_HOSTS` in the inline `env:` block, matching the three test-env Vite dev-server origins (`http://localhost:3050/3051/3052`). Without it, browser-based feature specs see the app but all API fetches fail with CORS errors, causing specs to time out with no useful diagnostic. The value mirrors what `EnvBuilder` already writes into `.env.test`; it was missing from the workflow's own env block.
- Generated CI workflows now use `go-version: 'stable'` instead of a pinned `go-version: '1.24'` when installing `oasdiff`. The generated app has no Go code, so reproducibility doesn't require pinning; using `stable` prevents `go install oasdiff@latest` from failing when a new oasdiff release requires a newer Go minor than the pinned version.
