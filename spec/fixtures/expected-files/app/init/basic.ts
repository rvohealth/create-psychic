...
export default async (psy: PsychicApp) => {
  ...
  await psy.load('initializers', srcPath('..', 'src', 'api', 'conf', 'initializers'), path => importDefault(path))
  ...
  psy.set('paths', {
    apiRoutes: 'src/api/conf/routes.ts',
    controllers: 'src/api/app/controllers',
    services: 'src/api/app/services',
  ...
  psy.set('openapi', {
    outputFilepath: path.join('src', 'api', 'openapi', 'openapi.json'),
    defaults: {
      components: {
        schemas: {},
      },
  ...
  psy.set('openapi', 'mobile', {
    outputFilepath: path.join('src', 'api', 'openapi', 'mobile.openapi.json'),
  ...
  psy.set('openapi', 'admin', {
    outputFilepath: path.join('src', 'api', 'openapi', 'admin.openapi.json'),
    defaults: {
      components: {
        schemas: {},
      },
  ...
  psy.set('openapi', 'internal', {
    outputFilepath: path.join('src', 'api', 'openapi', 'internal.openapi.json'),
    defaults: {
      components: {
        schemas: {},
      },
  ...
  psy.set('openapi', 'tests', {
    outputFilepath: path.join('src', 'api', 'openapi', 'tests.openapi.json'),
    syncTypes: true,
    defaults: {
      components: {
        schemas: {},
