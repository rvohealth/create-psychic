...
// scaffold path and needs its own spec. react covers the `bun create vite` path and
// the `bun run --cwd=../client dev` wrapper.
describe.skipIf(!bunAvailable())('newPsychicApp with a bun runtime and a react client', () => {
  it('provisions a react client with bun and runs its specs through bun', async () => {
